...
- Repositories are entered as separate GitHub owner and repository name fields; “Open on GitHub” links to the resulting repository and optional branch. Sync mode provides links for both repositories, can swap Repository A and Repository B before submission, and defaults to `git push --all` while retaining `git push --mirror` as an option.
